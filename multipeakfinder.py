            ...
            for _f in as_completed(_futures):
                if cwt_plot:
                    ...
                    for k in _cwt_list.keys():
                        ...
                        pos = ax_cwt1.imshow(_cwt, extent=[signal['time'][_xlim[0]]*1e-12, signal['time'][_xlim[0]]*1e-12+_cwt.shape[1]*dt, scales[0]*1e3, scales[-1]*1e3], origin='lower', cmap='inferno')
                        ax_cwt1.set_yticks([])
                        ax_cwt1.axis('auto')
                        ...
                        ax_cwt2.plot(signal['time'][_xlim[0]]+_result[0]['time'], _result[0]['scale']*1e3, '.', color='green')
                        ax_cwt2.set_ylim(scales[0]*1e3,scales[-1]*1e3)
                        ax_cwt1.set_xlim(signal['time'][_xlim[0]]*1e-12, signal['time'][_xlim[0]]*1e-12+_cwt.shape[1]*dt)
                        if log:
