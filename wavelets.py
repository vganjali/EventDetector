...
def mmi_gaussian(scale, N, window=1, weight=1, mod=0.5, shift=1, dt=1):
    ...
    if (type(N) == list):
        for i,n in enumerate(N):
            for m in range(n):
                s += weight[i]*np.exp(
                    -(t-length/2+((n-1)/2-m)*(N_max/n)*resolution)**2/(2*(sigma*N_max/n)**2))
    else:
        for m in range(N):
            s += weight*np.exp(
                -(t-length/2+((N_max-1)/2-m)*resolution)**2/(2*(sigma)**2))
    s -= np.amax(s)/window*np.exp(
        -(t-length/2+(N_max/2+shift/2)*resolution)**2/(2*(N_max/2*window*sigma)**2))
    s -= np.amax(s)/window*np.exp(
        -(t-length/2-(N_max/2+shift/2)*resolution)**2/(2*(N_max/2*window*sigma)**2))
    s -= np.mean(s)
    s_square_norm = np.trapz(s**2, dx=dt)
...
def mmi_gaussian_skewed(scale, N, window=1, weight=1, mod=0.5, shift=1, skewness=1, dt=1):
    ...
    for i,n in enumerate(N):
        for m in range(n):
            s += weight[i]*skew_normal(
                t,
                length/2+((n-1)/2-m)*(N_max/n)*resolution,
                sigma*N_max/n,
                alpha=0)
    s -= N_max/2*skew_normal(
        t,
        length/2+(N_max/2+shift/2)*resolution,
        (1+4*skewness)*N_max/2*window*sigma,
        alpha=4*skewness*N_max/2*window)
    s -= N_max/2*skew_normal(
        t,
        length/2-(N_max/2+shift/2)*resolution,
        (1+4*skewness)*N_max/2*window*sigma,
