...
class ptu(QObject):
	...
	def processHT2(self, update=False, relim=False):
		...
		self.timeout = 5
		# self.buffer = max(self.buffer, 10000000)
			...
			with open(self.filename, mode='rb') as fin:
				...
				while self.timeout > 0:
					try:
						...
						overflowCorrection = np.zeros(self.buffer, dtype=np.uint64)
		...
		with open(self.filename, mode='rb') as fin:
			...
			while self.rt_active:
				...
				if len(records) > 0:
					...
					buffer['time'] = binned_sparse_time[:-1]*self.binsize*1e12
					bin_edge['time'], bin_edge['count'] = binned_sparse_time[-1], binned_sparse_count[-1]
